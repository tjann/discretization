...
alpha <- 50
#Which node do you want to look at?
beta <- 10
#Gold Standard Network
goldFile <- "ADHD_Sub010fmri.csv"
#Discretized Results
method <- "ADHD_Sub010_twin.csv"
```
...
```{r}
gold <- read.tcsv(goldFile, header=T)[1:alpha,]
gold <- na.omit(gold)
#head(raw)
# raw %>% ggplot(aes(x = ))
...
discretized = discretized - min(discretized)
discretized = discretized / (max(discretized) - min(discretized))
#head(discretized)
```
