---
title: "Localized Area of Multiple Discretization Results for One Data Set"
author: "Tiffany Jann"
date: ""
output: 
  html_document:
...
```{r}
scoresPoint = apply(pointData, 2, diffArea, standard=gold) #apply diffArea to each point method's data set
scoresIntvl = apply(intervalData, 2, diffArea, standard=intvlGold) #apply diffArea to each interval method's data set
scores = append(scoresPoint, scoresIntvl) #join results for sorting
```

View results, best method first
```{r}
sort(scores) #display sorted results
```